...
export const conf = {
  ...
  network: 'MainNet',
  algod: {
    host: 'https://voitest-api.algorpc.pro',
    port: '',
    token: '',
  },
};
...
export async function collect(
  ...
  let myAlgo = Pipeline.pipeConnector === 'myAlgoWallet';
  ...
  if (!myAlgo) {
    alert('Close this alert then sign in your official Algorand Wallet');
  }
...
export async function getAsaId(escrow: string): Promise<number> {
  ...
  if (ai['assets'].length !== 1)
    throw Error('Expected 1 ASA for ' + escrow + ' got: ' + ai['assets'].length.toString());
