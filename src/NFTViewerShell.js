...
class App extends Component {
  ...
  deploy = async () => {
    ...
    Pipeline.deployTeal(
      ...
      [1, 1, 0, 6],
      [lastRound, lastRound + 10000, lastRound, lastRound + 10000],
    ).then((data) => {
  ...
  optIn = async () => {
    ...
    Pipeline.optIn(document.getElementById('appid').value, args).then((data) => {
      this.setState({ txID: data });
    });
  };
  ...
  vote = async () => {
    if (!document.getElementById('check').checked) {
      ...
      Pipeline.getAppCreator(appId).then((data) => {
        Pipeline.appCallWithTxn(appId, ['vote', 'candidateb'], data, 1, undefined, ASA).then(
          (data) => {
            this.setState({ txID: data });
          },
        );
  ...
  modifyTeal = () => {
    ...
    for (let i = 0; i < 2; i++) {
      tealContracts['Permissioned Voting'].program = tealContracts[
        'Permissioned Voting'
      ].program.replace(replacements[i], '// hard-coded assetid\nint ' + assetId);
      console.log(tealContracts['Permissioned Voting'].program);
    }
  };

  checkVote = async () => {
    Pipeline.readGlobalState(document.getElementById('appid').value).then((data) => {
      let keyIndex = '';
    ...
    return (
      <>
        <Navbar bg="dark" expand="lg" variant="dark">
          <Container>
            <Navbar.Brand href="#home">Voi Portfolio Tracker</Navbar.Brand>
